...
def _build_token_selection_cfg(cfg: Any) -> Optional[TokenSelectionConfig]:
    ...
    return TokenSelectionConfig(
        ...
        flops_profile_enabled=bool(getattr(cfg, "flops_profile_enabled", False)),
        flops_profile_silent=bool(getattr(cfg, "flops_profile_silent", True)),
        ...
        grad_score_method=str(getattr(cfg, "grad_score_method", "full_grad")),
        partial_grad_phi=str(getattr(cfg, "partial_grad_phi", "l2")),
        partial_grad_pos_weight=float(getattr(cfg, "partial_grad_pos_weight", 1.0)),
        partial_grad_grip_weight=float(getattr(cfg, "partial_grad_grip_weight", 2.0)),
        attn_score_beta=float(getattr(cfg, "attn_score_beta", 1.0)),
        head_consensus_gating_enabled=bool(getattr(cfg, "head_consensus_gating_enabled", False)),
        head_consensus_eta=float(getattr(cfg, "head_consensus_eta", 1.0)),
        head_g_beta=float(getattr(cfg, "head_g_beta", 1.0)),
        head_g_norm=str(getattr(cfg, "head_g_norm", "sum")),
        token_reuse_mode=str(getattr(cfg, "token_reuse_mode", "none")),
